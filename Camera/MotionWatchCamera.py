...
def beginWatching(notifyUserFunc=None):
	...
	camera = PiCamera()
	camera.resolution = (conf["res_x"], conf["res_y"])
	...
	for f in camera.capture_continuous(rawCapture, format="bgr", use_video_port=True):
		...
		if avg is None:
			print "[INFO] starting motion watch."
		...
		for c in cnts:
			...
			print("image contour diff: "+str(area))
			if area < min_area:
				continue
		...
		if text == "Occupied":
			...
			if (timestamp - lastUploaded).seconds >= conf["min_upload_seconds"]:
				...
				if motionCounter >= conf["min_motion_frames"]:
					...
					if(notifyUserFunc is not None):
						notifyUserFunc(t.path)
					else:
						print(t.path)
					t.cleanup()
